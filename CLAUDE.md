...
Siyahi ("ink") is Saumitra S. Phatak's static, multilingual poetry archive: 133 original poems (plus ~33 short aphorisms) in Hindi, Marathi, and English, presented as a searchable/browsable website with a separate original "book edition" view.
...
- No build tools, no linter, no tests — verify changes by opening `index.html`/`book.html` directly in a browser and/or running the Python script and checking its stdout ("Wrote 133 poems and N aphorisms...").
...
2. Regenerate the data file:
   ...
   python3 scripts/build_content.py
   ```
3. Confirm it printed `Wrote 133 poems and ... aphorisms to .../js/poems-data.js` with no error.
